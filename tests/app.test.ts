...
describe("app routes", () => {
  it("returns health status", async () => {
    const app = createApp({ runtime: createRuntimeMock() });
    const response = await app.request("/health");

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      ok: true,
      services: { ai: true, telegram: true },
    });
  });
  ...
  it("delegates webhook handling and forwards waitUntil", async () => {
    ...
    const response = await app.fetch(
      new Request("http://localhost/webhooks/telegram", { method: "POST" }),
      runtime.env,
  ...
  it("clears session state through the debug reset route", async () => {
    ...
    const app = createApp({
      runtime: createRuntimeMock({
        sessionStore: { ...baseRuntime.sessionStore, reset },
        state: { ...baseRuntime.state, unsubscribe },
      }),
    });
